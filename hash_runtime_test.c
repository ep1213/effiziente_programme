...
__attribute__((noinline)) unsigned long hash(char *addr, size_t len)
{
...
__attribute__((noinline)) int lookup_collision(char *keyaddr, size_t keylen, long start) {
  long i = (start + 1) & (HASHSIZE - 1);
...
__attribute__((noinline)) int lookup(char *keyaddr, size_t keylen)
{
  long i = hash(keyaddr, keylen) & (HASHSIZE-1);
  struct hashnode p = ht[i];

  if (keylen != p.keylen || memcmp(keyaddr, p.keyaddr, keylen)!=0) {
    return lookup_collision(keyaddr, keylen, i);
  }
