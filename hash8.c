...
void insert_bulk(struct insert_t *inserts, int size)
{
  int i;
  long hashes[size];
...
void lookup_bulk(struct lookup_t *lookups, int *values, int size)
{
  int i;
  long hashes[size];

  for (i = 0; i < size; i++) {
    hashes[i] = hash(lookups[i].keyaddr, lookups[i].keylen) & (HASHSIZE-1);
    _mm_prefetch(&ht[hashes[i]], _MM_HINT_T2);
  }

  for (i = 0; i < size; i++) {
    values[i] = lookup_sub(lookups[i].keyaddr, lookups[i].keylen, hashes[i]); 
  }
}
...
int main(int argc, char *argv[])
{
  ...
  struct insert_t insert_requests[BLOCKSIZE];
  for (p=input1.addr, endp=input1.addr+input1.len, i=0; p<endp;) {
    for (j = 0; j < BLOCKSIZE && p < endp; j++, i++) {
      ...
      insert_requests[j].keyaddr = p;
      insert_requests[j].keylen = nextp - p;
      insert_requests[j].value = i;

      p = nextp+1;
    }

    insert_bulk(insert_requests, j);
  }
#if 0 
 struct hashnode *n;
  ...
  struct lookup_t lookup_requests[BLOCKSIZE];
  int lookup_results[BLOCKSIZE];
  ...
  for (i=0; i<10; i++) {
    for (p=input2.addr, endp=input2.addr+input2.len; p<endp; ) {
      for (j = 0; j < BLOCKSIZE && p < endp; j++) {
        ...
        lookup_requests[j].keyaddr = p;
        lookup_requests[j].keylen = nextp - p;
        p = nextp+1;
      }
      lookup_bulk(lookup_requests, lookup_results, j);
      for (k = 0; k < j; k++) {
        r = ((unsigned long)r) * 2654435761L + lookup_results[k];
        r = r + (r>>32);
      }
    }
  }
